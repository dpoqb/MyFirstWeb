...
<table class="table table-striped table-hover">
	...
	<?php
		include "../front_lib/connc.user.php";                              	//包含数据库连接文件，连接数据库
		include "../front_lib/page.class.php";                               //包含分页类文件，加数据分页功能
		...
		if($result) {
			...
			while(list($id, $article_title, $first_author_name, $second_author_name, $periodical, $ptime, $pdf) = $result->fetch(PDO::FETCH_BOTH)) {
                ...
                echo '<td><a href="../works_lib/front_web.php?action=watch&id='.$id.'"><span class="glyphicon glyphicon-zoom-in" title="查看"></span></a></td>';
				echo '</tr>';
			}
